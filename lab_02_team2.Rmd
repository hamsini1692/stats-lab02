...
ggplot(movie_sample_exploration, aes(x=budget/1000000, y=revenue/1000000)) +
...
movie_sample_statistical_clean <- na.omit(movie_sample_statistical)
nrow(movie_sample_statistical_clean)
# create dummy variables for genres
#dummy_genres <- model.matrix(~0 + movie_sample_statistical_clean$genre_category)

#extract month from release_date
#movie_sample_statistical_clean$release_month <- as.numeric(format(as.Date(movie_sample_statistical_clean$release_date), "%m"))


#dummy_month <- model.matrix(~0 + movie_sample_statistical_clean$release_month)


#movie_sample_statistical_clean <- movie_sample_statistical_clean %>%
#  select(-release_month) %>%
#  bind_cols(dummy_month)



movie_sample_statistical_clean <- movie_sample_statistical_clean %>% 
  mutate(
    model_preds = predict(model_budget_genres_release), 
    model_resid = resid(model_budget_genres_release)
  ) 

#Check for correlations in all variables and figure out which one to drop 

#movie_sample_statistical_clean %>% 
#  select(budget,total_cast, vote_count, dummy_month, total_production_companies, title_length, model_resid) %>% 
#  GGally::ggpairs()

# create the regression model
model_budget_genres_release <- lm(revenue ~ budget + total_cast + vote_count + total_production_companies + title_length + runtime , data = movie_sample_statistical_clean)


#model_budget_genres_release$aliased
model_budget_genres_release$coefficients

get_robust_se <- function(model) {
  se <- sqrt(diag(vcovHC(model)))
  return(se)
}
#rse <- get_robust_se(model_budget_genres_release)

# display model summary
summary(model_budget_genres_release)
```

```{r statstesttotal_cast }
regression_model_1 <- lm(revenue ~ budget + total_production_companies + vote_count + title_length, data = movie_sample_statistical_clean)


regression_model_2 <- lm(revenue ~ budget + total_cast + vote_count + total_production_companies + title_length , data = movie_sample_statistical_clean)


#rse <- coeftest(regression, vcov = vcovHC)
#rse
#rse1 <- coeftest(regression1, vcov = vcovHC)
#rse1


coeftest(regression_model_1, vcov = vcovHC(regression_model_1))
coeftest(regression_model_2, vcov = vcovHC(regression_model_2))
summary(regression_model_1)
summary(regression_model_2)

#waldtest(regression, regression1, vcov = vcovHC(regression, type = "HC0"))
```


```{r results='asis', echo=FALSE, warning=FALSE, results='hide'}
#This function returns the robust standard error
model <- lm(revenue ~ budget + total_production_companies + vote_count + title_length, data = movie_sample_statistical_clean)
get_robust_se <- function(model) {
  se <- sqrt(diag(vcovHC(model)))
  return(se)
}
rse <- get_robust_se(model)
#Stargazer table along with robust standard error
#Stargazer table along with robust standard error
stargazer(model, type = "text", omit.stat = "f",se=list(rse), star.cutoffs = c(0.05, 0.01, 0.001),
...
selected_columns <- movie_sample_exploration %>%
  dplyr::select(budget, revenue, runtime, vote_count, vote_average, title_length, total_cast, popularity,total_production_companies)
...
```{r estimate models}
library(car)
model_true <- lm(revenue ~ budget + total_cast + vote_count + total_production_companies, 
                 data = movie_sample_statistical)
coeftest(model_true)
```
...
movie_sample_statistical %>% 
  select(budget, total_cast, vote_count, total_production_companies, model_resid) %>% 
  GGally::ggpairs()
```
...
library(stargazer) 
library(sandwich)
stargazer(model_true, type="text", omit.stat = 'all')
```
