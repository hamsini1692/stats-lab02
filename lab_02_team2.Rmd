...
result_corr <- corrplot(cor_matrix, method = "color",  order = "hclust", 
         tl.col = "black", tl.srt = 45, type = "upper", tl.cex = 0.6, title = "Correlation, Plot", title.cex=0.8,mar = c(1, 1, 3, 1))
...
```{r}
library(lubridate)
# Add holidays as a new column
movie_sample_statistical$release_date_cat <- ifelse(month(movie_sample_statistical$release_date) == 1 & day(movie_sample_statistical$release_date) == 1, 1, 0) |  # New Year's Day
              ifelse(month(movie_sample_statistical$release_date) == 12 & day(movie_sample_statistical$release_date) == 25, 1, 0) | # Christmas
              ifelse(month(movie_sample_statistical$release_date) == 11 & wday(movie_sample_statistical$release_date) == 5 & day(movie_sample_statistical$release_date) > 21, 1, 0) | # Thanksgiving (4th Thursday in November)
ifelse(month(movie_sample_statistical$release_date) == 02 & day(movie_sample_statistical$release_date) == 14, 1, 0) | # Valentine's day
ifelse(month(movie_sample_statistical$release_date) == 10 & day(movie_sample_statistical$release_date) == 31, 1, 0) | # Halloween's day
  
              ifelse(month(movie_sample_statistical$release_date) == 4 & wday(movie_sample_statistical$release_date) == 1 & (day(movie_sample_statistical$release_date) == 8 | day(movie_sample_statistical$release_date) == 9 | day(movie_sample_statistical$release_date) == 10 | day(movie_sample_statistical$release_date) == 11 | day(movie_sample_statistical$release_date) == 12 | day(movie_sample_statistical$release_date) == 13 | day(movie_sample_statistical$release_date) == 14), 1, 0) # Easter Sunday

# Create a new column indicating whether the date is a weekend, weekday, or holiday
movie_sample_statistical$release_date_cat <- ifelse(wday(movie_sample_statistical$release_date) %in% c(6,7,1), "1", "0")
#movie_sample_statistical$release_date_cat <- ifelse(movie_sample_statistical$holiday == 1, "1", movie_sample_statistical$release_date)
View(movie_sample_statistical)
```
...
ggplot(movie_sample_statistical, 
...
model_5 <- lm(log_revenue ~ log_budget + vote_count + log_title_length + runtime + popularity + release_date_cat, data = movie_sample_statistical)
...
rse <- get_robust_se(model_5)
#Stargazer table along with robust standard error
#Stargazer table along with robust standard error
stargazer(model_5, type = "text", omit.stat = "f",se=list(rse), star.cutoffs = c(0.05, 0.01, 0.001),
