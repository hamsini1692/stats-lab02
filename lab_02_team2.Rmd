...
result_corr <- corrplot(cor_matrix, method = "color",  order = "hclust", 
         tl.col = "black", tl.srt = 45, type = "upper", tl.cex = 0.6, cl.cex=0.6, title = "Figure 1. Correlation Plot", title.cex=0.8, mar = c(1, 1, 1, 1))
...
ggdraw() +
  draw_plot(result_corr, x = 0.6, y = 0.5, width = 0.4, height = 0.6) + 
  draw_plot(main_plot)
...
- release_season (binary): Many film studios release their biggest and most anticipated movies during during Christmas and New Year to take advantage of the increased audience turnout and box office revenue potential. Here we considered two prominent holiday months (December & January) as one category and rest of the months as a different category.
...
Before running the regression model, we checked the mutli-collinearity between all predictors by running a variation inflation factor to reduce unstable and unreliable estimates of the regression analysis and observed there is no evidence of mutli-collinearity between the predictor variables. 
