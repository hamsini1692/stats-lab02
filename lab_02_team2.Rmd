...
After the exploratory analysis, three models are built for the 70% dataset and regression analysis are carried out to find the influence of budget over the revenue of a movie. Table 1 shows the results of three regression models. 

From the table, the co-efficient of the X-concept variable "Budget" is having high statistical significance in all the three models studied. Its point estimates ranging from 0.9 to 0.7. 

his means that, on average, the model predicts that for every one million dollar increase in the movie budget, the revenue of the movie is expected to increase by approximately $774,800.

This also shows that effect of budget is absorbed by other co-variates. When we say, "absorbed" we mean that there is positive co-variance between the other co-variates in influencing the revenue.

A stepwise selection method was used to select variables for the model, with a significance level of p < .05 for inclusion. We have also conducted coeftest to understand the significance of all the co-variates considered in the model. The results shows that the co-variates vote count, runtime and release season show more significance when compared to other co-variates. We have also conducted Wald test to find out the significance of single co-variate in the regression model. The results shows that the variables vote count, run time, popularity and release season shows more significance when compared to Movie Title Length and Release Language. This is also evident from the Table 1.
From the table 1, we can see that Model 3 has the highest R-squared value, indicating that it is the best fitting model of the three.
...
stargazer(model, model_2, model_5,
          ...
          notes.append = FALSE,
          notes = "\\parbox[t]{7cm}{*p<0.1; **p<0.05; ***p<0.01. $HC_1$ robust standard errors specified in parentheses. Holiday season is Dec/Jan and rest of the months are considered as non-holiday season. Language Category is either English or Non-English movies.}"
          )
```
## Limitations
